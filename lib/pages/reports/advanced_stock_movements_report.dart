    ...
    final items = itemIds.where((id) => itemsMap.containsKey(id)).map((id) {
      ...
      return {
        ...
        'name': isArabic
            ...
            : (data['nameEn'] ?? data['nameAr'] ?? id),
      };
    }).toList();

    items.sort((a, b) => a['name'].compareTo(b['name']));

    setState(() => _items..clear()..addAll(items));
  }
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      for (final doc in movementsDocs) {
        ...
        if (isIn) _totalIn += quantity;
        if (isOut) _totalOut += quantity;
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: _movementTypes.map((type) {
                      return DropdownMenuItem<String>(
                        value: type,
                        child: Text('movement_type_$type'.tr()),
                      );
                    }).toList(),
                    onChanged: (val) {
                      setState(() {
