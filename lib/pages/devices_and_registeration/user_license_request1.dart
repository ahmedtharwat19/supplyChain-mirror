...
/*   Future<void> _loadCurrentDeviceId() async {
    try {
      ...
      if (kIsWeb) {
        ...
        final androidInfo = await _deviceInfo.androidInfo;
        // 🛡️ استخدام حقول متسلسلة لضمان عدم رجوع قيمة null نهائياً
        deviceId = androidInfo.id;
        if (deviceId.isEmpty || deviceId == 'null') {
          deviceId = androidInfo.hardware.hashCode.toString();
        }
      } else if (defaultTargetPlatform == TargetPlatform.iOS) {
        final iosInfo = await _deviceInfo.iosInfo;
      ...
      if (mounted) {
        setState(() {
          _currentDeviceId = deviceId; // ✅ إسناد المتغير دون خطأ Null
        });
        safeDebugPrint('📱 Current secure device ID loaded: $_currentDeviceId');
      }
    } catch (e) {
      safeDebugPrint('❌ Error loading device ID: $e');
      if (mounted) {
