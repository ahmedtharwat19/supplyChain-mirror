...
class _CompaniesPageState extends State<CompaniesPage> {
  ...
  // ======================== التهيئة من Hive أولاً ========================

  Future<void> _initializeFromHiveFirst() async {
    safeDebugPrint('📦 Loading companies from Hive first...');
  ...
  Future<void> _loadCompaniesFromHive() async {
    try {
      ...
      final cachedCompanyIds = await HiveService.getCachedData('user_company_ids');
        ...
        if (mounted) {
          setState(() {
            ...
            userCompanyIds = cachedCompanyIds is List
                ? cachedCompanyIds.map((id) => id.toString()).toList().cast<String>()
                : [];
            _lastHiveUpdate = DateTime.now();
          });
        }
        safeDebugPrint('✅ Companies loaded from Hive: ${_companies.length}, last update: $_lastHiveUpdate');
        return;
      }

      safeDebugPrint('🔄 No companies found in Hive, will fetch from Firestore');
      _lastHiveUpdate = null;
    } catch (e) {
  ...
  bool _shouldFetchFreshData() {
    if (_companies.isEmpty) {
      safeDebugPrint('📦 No companies in Hive, need to fetch');
      return true;
    }
    if (_lastHiveUpdate == null) {
      safeDebugPrint('📦 No last update time, need to fetch');
      return true;
    }
    final age = DateTime.now().difference(_lastHiveUpdate!);
    if (age > _cacheDuration) {
      safeDebugPrint('📦 Cache expired (${age.inMinutes} min > ${_cacheDuration.inMinutes} min), need to fetch');
      return true;
    }
    safeDebugPrint('✅ Cache is fresh (${age.inMinutes} min old), skipping fetch');
    return false;
  }
  ...
  void _startBackgroundUpdatesIfNeeded() {
    if (_shouldFetchFreshData()) {
      ...
      safeDebugPrint('✅ Hive companies data is fresh, skipping Firestore fetch');
    }
  }
  ...
  Future<void> _saveCompaniesToHive(List<Map<String, dynamic>> companies, List<String> companyIds) async {
    try {
      await HiveService.cacheData('companies', companies);
      await HiveService.cacheData('user_company_ids', companyIds);
      safeDebugPrint('💾 Companies saved to Hive');
    } catch (e) {
      safeDebugPrint('❌ Hive save error: $e');
    }
  }
  ...
  Future<void> _refreshData() async {
    if (mounted) setState(() => _isDataLoading = true);
    try {
      // إعادة جلب companyIds من Hive أولاً
      final userData = await HiveService.getUserData();
      if (userData != null && userData['companyIds'] is List) {
        setState(() {
          userCompanyIds = (userData['companyIds'] as List).cast<String>();
      ...
      // جلب جديد من Firestore وتحديث Hive
  ...
  Future<void> _confirmDeleteCompany(String companyId, Map<String, dynamic> companyData) async {
    ...
    if (confirmed == true) {
      try {
        ...
        // تحديث Hive
        await _removeCompanyFromLocal(companyId);
...
Future<void> _editCompany(String companyId) async {
  final result = await context.push('/edit-company/$companyId');
  if (result == true) {
    // ✅ إعادة تحميل البيانات من Hive مباشرة
    await _loadCompaniesFromHive();
        await _refreshData(); // تجلب من Firestore وتحدث Hive
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
        //  final result = await context.push('/add-company');
          final Map<String, dynamic>? result = await context.push('/add-company');

           if (result != null && result['success'] == true) {
      // ✅ إعادة تحميل البيانات من Hive بعد الإضافة
      await _loadCompaniesFromHive();
      if (mounted) setState(() {});
    }
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(tr('updating_data'), style: const TextStyle(fontSize: 14, color: Colors.blue, fontWeight: FontWeight.w500)),
          ],
        ),
      ),
    );
