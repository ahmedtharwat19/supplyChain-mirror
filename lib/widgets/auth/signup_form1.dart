...
/* void _signup() async {
  ...
  void _signup() async {
    ...
    try {
      ...
      // 2️⃣ إنشاء user document — trialUsed: false مؤقتاً عشان الـ rule تسمح بإنشاء license
      await FirebaseFirestore.instance
          .collection('users')
          .doc(createdUser.uid)
          .set({
        ...
        'isAdmin': false,
        'createdAt': FieldValue.serverTimestamp(),
        'trialUsed': false, // ✅ مؤقت - سيتغير بعد إنشاء الـ license
      });

      // 3️⃣ إنشاء الـ license التجريبي
      final licenseRef =
          FirebaseFirestore.instance.collection('licenses').doc();
      await licenseRef.set({
        'userId': createdUser.uid,
        'isTrialLicense': true, // ✅ علامة التجربة
        'isActive': true,
        'createdAt': FieldValue.serverTimestamp(),
        ...
        'devices': [],
        'deviceIds': [],
        'requestId': 'trial_${DateTime.now().millisecondsSinceEpoch}',
      });

      final licenseKey = licenseRef.id;

      // 4️⃣ تحديث المستخدم: ربط الـ license وتأكيد استخدام التجربة
      await FirebaseFirestore.instance
          .collection('users')
          .doc(createdUser.uid)
          .update({
        ...
        'trialUsed': true, // ✅ لن يستطيع إنشاء license تاني بنفسه
        'trialExpiryDate': Timestamp.fromDate(expiryDate),
        'maxDevices': 1,
      });
      ...
      await secureStorage.write(key: 'license_status', value: 'active');
      await secureStorage.write(
          key: 'license_cached_at', value: DateTime.now().toIso8601String());
      ...
      // ✅ Rollback: حذف حساب Auth لو فشل Firestore
      if (createdUser != null) {
        try {
