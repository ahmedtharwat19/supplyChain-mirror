...
class ReportsDataService {
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // الأصناف (Items) - مع تحويل Timestamp
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        itemsMap[doc.id] = {
          'nameAr': data['nameAr'] ?? doc.id,
          'nameEn': data['nameEn'] ?? doc.id,
          'name': data['name'] ?? data['nameEn'] ?? doc.id,
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // مخزون المصنع - مع تحويل Timestamp إلى String
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Future<Map<String, Map<String, dynamic>>> getFactoryInventory(
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        dynamic expiryDate = data['expiryDate'];
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // حركات المخزون - لا نحتاج كاش هنا لأنها متغيرة
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    try {
      ...
      for (final supplierId in userSupplierIds) {
        ...
        if (vendorCompanyIds.isEmpty || vendorCompanyIds.contains(companyId)) {
          suppliers.add({
            'id': supplierId,
            'nameAr': data['nameAr'] ?? supplierId,
            'nameEn': data['nameEn'] ?? supplierId,
          });
        }
      }
  ...
  Future<void> _saveToCache(String key, dynamic data) async {
    try {
      ...
      // ✅ تحويل آمن إلى JSON
      final jsonString = json.encode(data, toEncodable: (obj) {
        // إذا كان Timestamp، حوله إلى String
        if (obj is Timestamp) {
          return obj.toDate().toIso8601String();
        }
        // إذا كان DateTime، حوله إلى String
        if (obj is DateTime) {
          return obj.toIso8601String();
        }
        // إذا كان Map أو List، دعه يمر
        return obj;
      });
