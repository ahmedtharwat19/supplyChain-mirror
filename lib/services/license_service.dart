  ...
  Future<LicenseStatus> checkLicenseStatus(String licenseKey) async {
    try {
      ...
      try {
        ...
        // حفظ البيانات للتخزين المؤقت
        await _cacheLicenseData(box, licenseKey, expiryDate, maxDevices,
...
class LicenseService {
  ...
  Future<LicenseStatus> checkLicenseStatus(String licenseKey) async {
    try {
      ...
      try {
        ...
        // ✅ الحصول على بيانات الترخيص الكاملة من licenses collection
        final licenseDoc = await _firestore.collection('licenses').doc(licenseKey).get();
  ...
  /// التحقق مما إذا كان يمكن تغيير الجهاز (مرة واحدة فقط)
  ...
  /// تغيير الجهاز الحالي (استبدال الجهاز القديم بالجديد)
  ...
  /// إصلاح التراخيص القديمة وإضافة الحقول الجديدة (للمسؤولين فقط)
  Future<void> migrateLicensesWithNewFields() async {
    try {
      final user = _auth.currentUser;
      if (user == null) return;
      ...
      final isAdmin = await _checkIfAdmin(user.uid);
      if (!isAdmin) {
        safeDebugPrint('Only admin can migrate licenses');
        return;
      }
      ...
      for (final doc in licenses.docs) {
        ...
        if (!data.containsKey('deviceChanged')) {
          updates['deviceChanged'] = false;
          needsUpdate = true;
        }
        if (!data.containsKey('canChangeDeviceAgain')) {
          updates['canChangeDeviceAgain'] = true;
          needsUpdate = true;
        }
        if (!data.containsKey('deviceChangeDate')) {
          updates['deviceChangeDate'] = null;
          needsUpdate = true;
        }
        if (!data.containsKey('originalDeviceFingerprint')) {
          updates['originalDeviceFingerprint'] = null;
          needsUpdate = true;
        }
        ...
        if (needsUpdate) {
          ...
          migratedCount++;
          safeDebugPrint('✅ Migrated license: ${doc.id}');
        }
      }
      
      safeDebugPrint('✅ License migration completed. Migrated: $migratedCount licenses');
    } catch (e) {
      safeDebugPrint('❌ Error migrating licenses: $e');
    }
  }
